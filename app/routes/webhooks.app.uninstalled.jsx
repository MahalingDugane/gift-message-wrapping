...
export const action = async ({ request }) => {
  ...
  switch (topic) {
    case "CUSTOMERS_DATA_REQUEST":
      // Acknowledge data request; customer data is not stored in Prisma
      return new Response("Customer data request acknowledged", { status: 200 });

    case "CUSTOMERS_REDACT":
      // Acknowledge redact request; customer data is not stored in Prisma
      return new Response("Customer redaction request acknowledged", { status: 200 });

    case "SHOP_REDACT":
      // Delete data strictly belonging to the authenticated shop
